const slides = ['#one', '#two', '#three', '#four', '#five', '#six', '#seven', '#eight', '#nine', '#ten', '#eleven', '#twelve', '#end'];
...
function checkAnswer(quizId, correctAnswer, feedbackId) {
  ...
  if (selectedAnswer === correctAnswer) {
    ...
    const submitBtn = document.querySelector('#nine .btn');
...
function resetQuiz(quizId, feedbackId) {
  ...
  submitBtn.onclick = function() { checkAnswer('quiz1', 'c', 'feedback1'); };
...
function submitAnswer() {
  ...
  if (isCorrect) {
    feedback.className = 'feedback correct';
    feedback.innerHTML = '🎉 Amazing Job! Google Uses the Mobile Version of Your Website to Determine Rankings (Mobile-First Indexing)';
  } else {
    feedback.className = 'feedback incorrect';
