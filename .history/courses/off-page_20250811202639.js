const slides = ['#one', '#two', '#three', '#four', '#five', '#six', '#seven', '#eight', '#nine', '#end'];
...
function checkAnswer(quizId, correctAnswer, feedbackId) {
  ...
  if (selectedAnswer === correctAnswer) {
    ...
    const submitBtn = document.querySelector('#six .btn');
...
function resetQuiz(quizId, feedbackId) {
  ...
  submitBtn.onclick = function() { checkAnswer('quiz1', 'b', 'feedback1'); };
...
const correctAnswer = ['Link', 'Building'];
...
function submitAnswer() {
  ...
  if (isCorrect) {
    feedback.className = 'feedback correct';
    feedback.innerHTML = '🎉 Nice! Link Building is a Factor That Drives More Traffic Into Your Website!';
  } else {
    feedback.className = 'feedback incorrect';
