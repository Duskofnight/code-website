...
function loadLessons() {
    document.querySelectorAll(".lesson").forEach((el, idx) => {
        el.classList.remove("locked");
        el.onclick = () => {
            completedLessons.push(lessons[idx]);
            console.log("Completed:", lessons[idx]);
            loadLessons(); // reload to unlock next
        };

        if (idx > 0 && !completedLessons.includes(lessons[idx - 1])) {
            el.classList.add("locked");
            el.onclick = () => alert("Complete previous lesson first!");
        }
    });
}
