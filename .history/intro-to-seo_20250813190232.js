...
function loadLessonProgress() {
    ...
    // Only the first lesson unlocked
    return {
        completed: [], 
        current: 'introduction', 
        unlocked: ['introduction'] // first lesson only
    };
}
...
function completeLesson(lessonId) {
    if (!lessonProgress.completed.includes(lessonId)) {
        ...
        // Optional: set current lesson to next
        if (nextLesson) lessonProgress.current = nextLesson;
