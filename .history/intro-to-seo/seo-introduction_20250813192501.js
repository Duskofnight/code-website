...
// Use DOMContentLoaded for better loading
...
function checkAnswer(quizId, correctAnswer, feedbackId) {
  ...
  if (selectedAnswer === correctAnswer) {
    ...
    // Hide try again button if it exists
    const tryAgainBtn = currentPage.querySelector('.submit');
    if (tryAgainBtn) {
    ...
    if (singleBtn) {
      ...
      singleBtn.style.background = '#f39c12';
...
function resetQuiz(quizId, correctAnswer, feedbackId) {
  ...
  if (singleBtn) {
    ...
    singleBtn.removeEventListener('mouseenter', function() {});
    singleBtn.removeEventListener('mouseleave', function() {});
  }
  
  // Show try again button if it was hidden
  const tryAgainBtn = currentPage.querySelector('.submit');
  if (tryAgainBtn) {
    tryAgainBtn.style.display = 'auto';
    tryAgainBtn.style.pointerEvents = 'auto';
  }
...
function checkAllBlanksFilled() {
  ...
  if (singleBtn) {
    if (allFilled) {
      singleBtn.classList.add('filled');
    } else {
...
function submitAnswer() {
  ...
  if (isCorrect) {
    ...
    // Show green on words if completely correct answer
